...
### Разрешение имён без РНОКПП (link-names)

name-only-реестры (wanted/missing/проф) хранят людей как `name`-сущности без РНОКПП.
Чтобы связать такого человека с `ipn`-идентичностью из базы, где РНОКПП есть
(должники, ФОП/ПДВ), служит консервативная команда:

```bash
python scripts/entity_links.py link-names --db links.db --min-score 0.95
```

Она сопоставляет **person-like** `name`-сущности с `ipn` по ФИО и добавляет ребро
`identity`/`name_resolve` только когда:
- фамилия совпадает **точно**;
- имя совпадает полностью или по однозначному инициалу;
- если отчество известно с обеих сторон — совпадает; иначе — слабее;
- лучший кандидат **однозначно** лучше второго (нет неоднозначности).

Это эвристика-подсказка: она никогда не создаёт идентификаторов и не обращается с
обезличиванием. Рёбра `name_resolve` можно отдельно отозвать:
