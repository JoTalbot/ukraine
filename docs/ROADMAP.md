...
- **SEC-01 — Automated security/privacy boundary scan:** continuously scan tracked repository text for high-confidence credentials and enforce explicit public-open-data/no-deanonymization policy markers and training-manifest privacy declarations. **Implemented and runtime-verified by Security scan #14 on commit `ac99b7ff9bfd87e8b47bf9da845c7f9de2168292`; unit tests passed and the security evidence contract completed green.**
- **SEC-02 — Security signal control-plane binding:** publish the SEC-01 result as a standard `security` status signal and consume it in Release Control Plane so the canonical snapshot reflects the actual scan result rather than an unconditional green value. **Implemented and runtime-verified; Security scan #26 emitted the signal and Release Control Plane #250 consumed it successfully.**
