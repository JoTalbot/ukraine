...
- **CHAIN-02 — Producer binding execution order:** any workflow that invokes `validate_release.py` must generate the required hardening evidence and execute `bind_release_artifacts.py` first, because CHAIN-01 is an executable release contract rather than documentation-only metadata. **Discovered by runtime CI on 2026-09-16; implementation is the next hardening step.**
