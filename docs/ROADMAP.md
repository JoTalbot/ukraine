...
- **SEC-02 — Security signal control-plane binding:** publish the SEC-01 result as a standard `security` status signal and consume it in Release Control Plane so the canonical snapshot reflects the actual scan result rather than an unconditional green value. **Implemented; deterministic tests added and workflow verification in progress.**
