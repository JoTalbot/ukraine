...
- **PROM-02 — Authenticated promotion event:** production authorization now emits a canonical SHA-256 `authorization_id` over the immutable authorization fields; verifier enforces exact 40-hex source commit, strict UTC timestamp, artifact/evaluation hashes, and authorization identity. The event is verified before upload and bound into CHAIN-01. **Implemented; runtime verification remains pending an actual authorization workflow execution.**
...
- **CI-02 — Repository lint integrity:** keep production validators and readiness tests clean under the repository's Ruff configuration, including required imports and explicit exception-handling constructs. **Implemented: Ruff violations fixed in HEAD (commit `9477672` for `scripts/validate_release.py`); runtime-verified — `ruff check --config ruff.toml scripts tests` = RC=0 on 2026-09-25.**
- **CHAIN-02 — Producer binding execution order:** any workflow that invokes `validate_release.py` must generate the required hardening evidence and execute `bind_release_artifacts.py` first, because CHAIN-01 is an executable release contract rather than documentation-only metadata. **Implemented: Release observability fixed in HEAD (commit `edd6516`); ordering verified in `release-observability.yml` — evidence → bind → validate, matching `ukraine-data-ci.yml` (2026-09-25).**
