...
def check(root: Path) -> dict:
    ...
    if manifest.is_file():
        try:
            ...
            manifest_valid = manifest_payload.get("schema_version") == 1 and bool(manifest_payload.get("git_commit")) and isinstance(manifest_payload.get("files"), list) and bool(manifest_payload["files"])
    ...
    if status.is_file():
        try:
            ...
            complete = signals == REQUIRED_SIGNALS; freshness = _valid_freshness_policy(policy)
            gates["control_plane"] = {"state": "green" if complete and freshness else "red", "signal_set_complete": complete, "freshness_policy_present": freshness, "overall_state": payload.get("overall_state", "unknown")}
    ...
    lock = next((path for path in (root / "requirements.lock", root / "requirements-ci.lock", root / "uv.lock") if path.is_file()), None)
    ...
    if evidence_path.is_file():
        try:
            ...
            contracts_ok = all(contract_states.get(name, {}).get("state") in {"green", "quarantined"} for name in HARDENING_CONTRACTS)
