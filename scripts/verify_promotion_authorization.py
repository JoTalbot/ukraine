...
def verify(root: Path) -> tuple[bool, list[str]]:
    ...
    if not auth_path.is_file():
        return False, [f"missing authorization: {AUTH}"]
    if not manifest_path.is_file():
        issues.append("missing release manifest")
    try:
        auth = json.loads(auth_path.read_text(encoding="utf-8"))
    except (OSError, UnicodeDecodeError, json.JSONDecodeError):
        return False, ["authorization is invalid JSON"]
    try:
    ...
    if artifact is not None and artifact.is_file() and valid_sha(auth.get("artifact_sha256")):
        if sha256(artifact) != auth["artifact_sha256"]:
            issues.append("production artifact checksum mismatch")
    elif artifact is None or not artifact.is_file():
        issues.append("production artifact is missing")
    if evaluation is not None and evaluation.is_file() and valid_sha(auth.get("evaluation_evidence_sha256")):
        if sha256(evaluation) != auth["evaluation_evidence_sha256"]:
...
def main() -> None:
    ...
    p.add_argument("--output", default="artifacts/status/promotion-authorization-verification.json")
    a = p.parse_args()
    ok, issues = verify(Path(a.root))
    out = Path(a.output)
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps({"schema_version": V, "state": "green" if ok else "red", "authorization": AUTH, "issues": issues}, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
